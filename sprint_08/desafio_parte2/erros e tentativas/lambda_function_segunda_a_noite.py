...
for movie_id in filtered_ids:
    ...
    # GRAVAR DADOS JSON NO BUCKET DA AWS
...
#s3 = boto3.resource('s3')
#bucket_name = 'natalias-s3-bucket'
#directory_name = f'Raw/Local/JSON/Movies/{datetime.now().strftime("%Y/%m/%d")}/{file_name}'
#file_name = 'tmdb_filtered_movies.json'
#
...
#s3.put_object(bucket_name, directory_name + file_name).put(Body=json_data)
